...
namespace FluorineFx.Util
{
    ...
    public abstract class DateTimeUtils
    {
        ...
        public static DateTime ParseInternetDate(string input)
        {
            ...
            var zoneSpaceIndex = input.LastIndexOf(' ');

            if (zoneSpaceIndex <= 0)
                throw new FormatException();

            var zone = input.Substring(zoneSpaceIndex + 1);
            ...
            var time = DateTime.ParseExact(input, _internetDateFormats, CultureInfo.InvariantCulture, DateTimeStyles.AllowInnerWhite);
