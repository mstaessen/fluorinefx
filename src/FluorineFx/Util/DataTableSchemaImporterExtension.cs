...
namespace FluorineFx.Util
{
    ...
    class DataTableSchemaImporterExtension : SchemaImporterExtension
    {
        ...
        {
            var values = schemas.GetSchemas(schemaNamespace);
            if (values.Count != 1)
            {
                return null;
            }
            var schema = values[0] as XmlSchema;
            if (schema == null)
                return null;
            var type = (XmlSchemaType)schema.SchemaTypes[new XmlQualifiedName(name, schemaNamespace)];
