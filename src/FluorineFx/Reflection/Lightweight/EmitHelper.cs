...
namespace FluorineFx.Reflection.Lightweight
{
	...
	class EmitHelper
	{
		...
		public EmitHelper call(Type type, string methodName, params Type[] parameterTypes)
		{
			...
			var mi = type.GetMethod(methodName, parameterTypes);
		...
		public EmitHelper call(Type type, string methodName, BindingFlags flags, params Type[] parameterTypes)
		{
			...
			var mi = type.GetMethod(methodName, flags, null, parameterTypes, null);
        ...
        */
		...
		public EmitHelper Init(ParameterInfo parameterInfo, int index)
		{
			...
			var type = TypeHelper.GetUnderlyingType(parameterInfo.ParameterType);
		...
		{
			var fi = _ilGenerator.GetType().GetField(
        ...
        public EmitHelper GenerateSetMember(MemberInfo memberInfo)
        {
            ...
            if (memberInfo.MemberType == MemberTypes.Property)
            {
                ...
                var miPropertySet = propertyInfo.GetSetMethod();
        ...
        public EmitHelper GeneratePrimitiveCast(TypeCode typeCode)
        {
            switch (typeCode)
            {
                ...
                case TypeCode.Decimal:
                    {
                        var miDecimalOpExplicit = typeof(Decimal).GetMethod("op_Explicit", new Type[] { typeof(double) });
        ...
        {
            var exceptionConstructor = typeof(UnexpectedAMF).GetConstructor(AnyVisibilityInstance, null, CallingConventions.HasThis, new Type[] { typeof(string) }, null);
        ...
        {
            var miFluorineConfigurationGetInstance = typeof(FluorineConfiguration).GetMethod("get_Instance");
            var miFluorineConfigurationGetNullableValues = typeof(FluorineConfiguration).GetMethod("get_NullableValues");
            var miTypeGetTypeFromHandle = typeof(Type).GetMethod("GetTypeFromHandle");
            var miNullableTypeCollectionGetItem = typeof(NullableTypeCollection).GetMethod("get_Item", new Type[] { typeof(Type) });
