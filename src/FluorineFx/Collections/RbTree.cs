...
namespace FluorineFx.Collections
{
    ...
    public class RbTree
    {
        ...
        private InsertResult BinaryInsert(RbTreeNode z, bool allowDuplicates, bool replaceIfDuplicate)
        {
            ...
            {
                y = x;
                var result = _comparer.Compare(x.Value, z.Value);
        ...
        {
            var newNode = new RbTreeNode(val);
            var result = BinaryInsert(newNode, allowDuplicates, replaceIfDuplicate);
        ...
        {
            var x = Root;
            var y = RbTreeNode.Nil; // previous value of x
        ...
        public void Erase(RbTreeNode z)
        {
            ...
            var zVal = z.Value as IDisposable;
            if (zVal != null)
                zVal.Dispose();
        }
        ...
        {
            var first = LowerBound(val);
            var last = UpperBound(val);
            var nDeleted = 0;
