...
namespace FluorineFx.IO.Bytecode.Lightweight
{
    ...
    class AMF3ReflectionOptimizer : IReflectionOptimizer
	{
        ...
        protected virtual ReadDataInvoker CreateReadDataMethod(Type type, AMFReader reader, object instance)
        {
            ...
            var miAddReference = typeof(AMFReader).GetMethod("AddAMF3ObjectReference");
            ...
            for (var i = 0; i < _classDefinition.MemberCount; i++)
            {
                var key = _classDefinition.Members[i].Name;
                var typeCode = reader.ReadByte();
                var value = reader.ReadAMF3Data(typeCode);
                ...
                else
                {
                    ...
                    emit
                        ...
                        .callvirt(typeof(AMFReader).GetMethod("ReadAMF3Data", new[] { typeof(byte) }))
                        .pop
                        .end()
                    ;
                }
        ...
        private void GeneratePropertySet(EmitHelper emit, int typeCode, MemberInfo memberInfo)
        {
            ...
            if (memberType.IsPrimitive || memberType == typeof(decimal))
            {
                ...
                switch (primitiveTypeCode)
                {
                    ...
                    case TypeCode.Double:
                        {
                            var labelCheckInt = emit.ILGenerator.DefineLabel();
                    ...
                    case TypeCode.Boolean:
                        {
                            var labelCheckBooleanTrue = emit.ILGenerator.DefineLabel();
            ...
            {
                var labelNotInteger = emit.ILGenerator.DefineLabel();
                var labelNotString = emit.ILGenerator.DefineLabel();
                var labelExit = emit.ILGenerator.DefineLabel();
            ...
            {
                var labelCheckNull = emit.ILGenerator.DefineLabel();
                var labelCheckUndefined = emit.ILGenerator.DefineLabel();
                var labelNotString = emit.ILGenerator.DefineLabel();
                var labelExit = emit.ILGenerator.DefineLabel();
