...
namespace FluorineFx.IO.Bytecode.CodeDom
{
	...
	class AMF0ReflectionOptimizer
	{
		...
		private void InitCompiler()
		{
			...
			var classAssembly = _mappedClass.Assembly;
            ...
            foreach (var referencedName in Assembly.GetExecutingAssembly().GetReferencedAssemblies())
            {
                var referencedAssembly = Assembly.Load(referencedName);
                AddAssembly(referencedAssembly.Location);
            }
		...
		private IReflectionOptimizer Build(string code)
		{
			...
			if( FluorineConfiguration.Instance.OptimizerSettings.Debug )
			{
				var file = Path.Combine(Path.GetTempPath(), _mappedClass.FullName.Replace('.', '_').Replace("+", "__") ) + ".cs";
				var sw = File.CreateText( file);
				sw.Write(code);
				sw.Close();
                log.Debug(String.Format(Resources.Optimizer_FileLocation, _mappedClass.FullName, file));
            ...
            {
#if !(NET_1_1)
                res = provider.CompileAssemblyFromSource( _cp, code);
#else
				ICodeCompiler compiler = provider.CreateCompiler();
			...
			if (res.Errors.HasErrors)
			{
				...
				{
					log.Error(String.Format(Resources.Compiler_Error, e.Line, e.Column, e.ErrorText));
				}
			...
			var optimizer = (IReflectionOptimizer)assembly.CreateInstance(types[0].FullName, false, BindingFlags.CreateInstance, null, null, null, null);
			return optimizer;
		}
	...
	";
		...
		protected virtual string GenerateCode(object instance)
		{
			...
			for (var typeCode = _reader.ReadByte(); typeCode != AMF0TypeCode.EndOfObject; typeCode = _reader.ReadByte())
			{
				_layouter.Append("key = reader.ReadString();");
				_layouter.Append("typeCode = reader.ReadByte();");

				var value = _reader.ReadData(typeCode);
                ...
                else
                {
                    ...
                    var msg = String.Format(Resources.Reflection_MemberNotFound, string.Format("{0}.{1}", _mappedClass.FullName, key));
                    log.Warn(msg);
