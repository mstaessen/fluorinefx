    ...
    public static class MethodHandler
    {
            ...
            bool throwException, bool traceError)
        {
            ...
            if (suitableMethodInfos.Count > 0) {
                ...
                for (var i = suitableMethodInfos.Count - 1; i >= 0; i--) {
                    var methodInfo = suitableMethodInfos[i];
            ...
            if (suitableMethodInfos.Count == 0) {
                var msg = String.Format(Resources.Invocation_NoSuitableMethod, methodName);
                if (traceError) {
#if !SILVERLIGHT
                    if (Log.IsErrorEnabled) {
                        ...
                        try {
                            ...
                            Log.Error(string.Format("Reflected type was '{0}' location of the loaded file {1}",
                                type.AssemblyQualifiedName, type.Assembly.Location));
                        } catch (SecurityException) {
                            Log.Error(
                                string.Format(
                                    "Reflected type was '{0}' location of the loaded file cannot be determined",
                                    type.AssemblyQualifiedName));
                        }
                        var suitableMethodInfosTmp = new List<MethodInfo>();
                        for (var i = 0; i < methodInfos.Length; i++) {
                            var methodInfo = methodInfos[i];
                            if (methodInfo.Name == methodName) {
                                if ((methodInfo.GetParameters().Length == 0 && arguments == null)
                        ...
                        for (var i = suitableMethodInfosTmp.Count - 1; i >= 0; i--) {
                            var methodInfo = suitableMethodInfosTmp[i];
                            var parameterInfos = methodInfo.GetParameters();
                            ...
                            for (var j = 0; j < parameterInfos.Length; j++) {
                                ...
                                if (!exactMatch) {
                                    if (!TypeHelper.IsAssignable(arg, parameterInfo.ParameterType)) {
                                        if (arg != null)
                                            Log.Error(string.Format("{0}({1}) did not match value \"{2}\" ({3})",
                                                parameterInfo.Name, parameterInfo.ParameterType.Name, arg,
                                                arg.GetType().Name));
                                        else
                                            Log.Error(string.Format("{0}({1}) did not match null)", parameterInfo.Name,
                        ...
                        for (int j = 0; arguments != null && j < arguments.Count; j++)
                        {
                            ...
                            string trace;
                            if (arg != null)
                                trace = String.Format(Resources.Invocation_ParameterType, j, arg.GetType().FullName);
                            else
                                trace = String.Format(Resources.Invocation_ParameterType, j, "null");
                            log.Error(trace);
                        }
                        */
                    }
            ...
            if (suitableMethodInfos.Count > 1) {
                var msg = String.Format(Resources.Invocation_Ambiguity, methodName);
                if (throwException)
                    throw new FluorineException(msg);
            }
            if (suitableMethodInfos.Count > 0)
